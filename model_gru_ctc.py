...
def get_gru_ctc_model(image_size = image_size,
    ...
    ###add for compute accuracy during training and validation
    #acc_out = Lambda(accuracy, output_shape=(1,), name = 'my_accuracy')([x, labels])
...
"""
#todo: use accuracy as a matrix
def accuracy(args):
    seq_len = 8
    y_pred, y_true = args
    shape = y_pred[:6, :, :].shape  # 2:
    print(shape)
    print(K.ctc_decode(y_pred[:6, :, :], input_length=np.ones(shape[0]) * shape[1])[0][0])

    out = K.get_value(K.ctc_decode(y_pred[:, :, :], input_length=np.ones(shape[0]) * shape[1])[0][0])[:, :seq_len]

    # out has shape [bs, seq_len]
    #y_true has shape [bs, seq_len]
    bs = K.shape(out)[0]
    correct_count = 0
    for i in range(bs):
        ok = True
        for j in range(seq_len):
            if out[i][j] != y_pred[i][j]:
                ok = False
                break
        if ok:
            correct_count += 1

    return K.tf.cast(1.0)
"""
